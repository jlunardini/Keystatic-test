...
var api = require('./api-882a871b.node.cjs.js');
var index$1 = require('./index-8044c959.node.cjs.js');
...
var useSlugsInCollection = require('./useSlugsInCollection-ca6054d4.node.cjs.js');
...
function validateUrl(validation, value, label) {
  if (value !== null && (typeof value !== 'string' || !languages.isValidURL(value))) {
...
function useImageDimensions(src) {
  ...
  React.useEffect(() => {
    if (!src || !languages.isValidURL(src)) {
      setDimensions({});
      return;
    }
    const img = new Image();
...
function ImageField(props) {
  ...
  const imageLibraryURL = api.useImageLibraryURL();
  ...
  const [lastPastedUrl, setLastPastedUrl] = React.useState(null);
  const onPaste = event => {
    event.preventDefault();
    const text = event.clipboardData.getData('text/plain');
    const parsed = api.parseImageData(text);
    setLastPastedUrl(parsed.src);
    props.onChange(parsed);
  };
  const onLoad = index$1.useEventCallback(img => {
    const dimensions = {
      width: img.width,
      height: img.height
    };
  ...
  React.useEffect(() => {
    ...
    if (!languages.isValidURL(urlForAutoDimensions)) {
      setStatus('');
      return;
    }
    setStatus('loading');
    ...
    children: [/*#__PURE__*/jsxRuntime.jsx(textField.TextField, {
      ...
      onKeyDown: e => {
        if (e.code === 'Backspace' || e.code === 'Delete') {
          props.onChange(api.emptyImageData);
        }
      },
