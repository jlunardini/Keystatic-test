...
var readLocal = require('../../../dist/read-local-d1c32dd1.node.react-server.cjs.js');
var path = require('path');
require('../../../dist/index-22034702.node.react-server.cjs.js');
...
function makeGenericAPIRouteHandler(_config, options) {
  ...
  const baseDirectory = path__default["default"].resolve((_config$localBaseDire = _config.localBaseDirectory) !== null && _config$localBaseDire !== void 0 ? _config$localBaseDire : process.cwd());
  const getParams = req => {
    let url;
    try {
      url = new URL(req.url);
...
async function githubOauthCallback(req, config) {
  ...
  if (cookies['ks-template']) {
    ...
    if (owner && repo) {
      ...
      const gqlRes = await fetchGraphQL(noTransform.gql`
      ...
      if (defaultBranchName && defaultBranchSha && configFile) {
        ...
        if (configFileText !== configFile.object.text) {
          await fetchGraphQL(noTransform.gql`
...
async function getTokenCookies(tokenData, config) {
  const headers = [['Set-Cookie', cookie__default["default"].serialize('keystatic-gh-access-token', tokenData.access_token, {
    ...
    path: '/'
  })], ['Set-Cookie', cookie__default["default"].serialize('keystatic-gh-refresh-token', await Iron__default["default"].seal(tokenData.refresh_token, config.secret, {
    ...Iron__default["default"].defaults,
    ...
    path: '/'
  })], ['Set-Cookie', cookie__default["default"].serialize('ks-template', '', {
    sameSite: 'lax',
    secure: process.env.NODE_ENV === 'production',
    maxAge: 0,
    expires: new Date(),
...
async function getRefreshToken(req, config) {
  ...
  let refreshToken;
  try {
    refreshToken = await Iron__default["default"].unseal(refreshTokenCookie, config.secret, Iron__default["default"].defaults);
  } catch {
    return;
  }
...
async function githubLogin(req, config) {
  ...
  const state = node_crypto.randomBytes(10).toString('hex');
...
const ghAppSchema = z__default["default"].object({
  slug: z__default["default"].string(),
  client_id: z__default["default"].string(),
  client_secret: z__default["default"].string()
});
...
async function update(req, config, baseDirectory) {
  ...
  const isFilepathValid = getIsPathValid(config);
  const updates = z__default["default"].object({
    additions: z__default["default"].array(z__default["default"].object({
      path: z__default["default"].string().refine(isFilepathValid),
      contents: z__default["default"].string().transform(x => Buffer.from(x, 'base64'))
    })),
    deletions: z__default["default"].array(z__default["default"].object({
      path: z__default["default"].string().refine(isFilepathValid)
    }))
