...
// EXTERNAL MODULE: ./node_modules/@keystatic/core/dist/index-cd5679e3.node.react-server.esm.js + 1 modules
var index_cd5679e3_node_react_server_esm = __webpack_require__(59937);
// EXTERNAL MODULE: ./node_modules/@sindresorhus/slugify/index.js
var slugify = __webpack_require__(34794);
// EXTERNAL MODULE: ./node_modules/@braintree/sanitize-url/dist/index.js
var dist = __webpack_require__(29382);
...
var emery_cjs = __webpack_require__(5106);
...
;// CONCATENATED MODULE: ./node_modules/@keystatic/core/dist/required-files-4d510aad.node.react-server.esm.js
...
function validateArrayLength(schema, val, path) {
    ...
    if (((_schema$validation = schema.validation) === null || _schema$validation === void 0 || (_schema$validation = _schema$validation.length) === null || _schema$validation === void 0 ? void 0 : _schema$validation.min) !== undefined && val.length < schema.validation.length.min) {
        return new PropValidationError(new index_cd5679e3_node_react_server_esm.F(`Must have at least ${schema.validation.length.min} element${schema.validation.length.min === 1 ? "" : "s"}`), path, schema);
    }
    if (((_schema$validation2 = schema.validation) === null || _schema$validation2 === void 0 || (_schema$validation2 = _schema$validation2.length) === null || _schema$validation2 === void 0 ? void 0 : _schema$validation2.max) !== undefined && val.length > schema.validation.length.max) {
        return new PropValidationError(new index_cd5679e3_node_react_server_esm.F(`Must have at most ${schema.validation.length.max} element${schema.validation.length.max === 1 ? "" : "s"}}`), path, schema);
    }
}
...
function parseProps(schema, _value, path, pathWithArrayFieldSlugs, parseFormField, /** This should be true for the reader and false elsewhere */ validateArrayFieldLength) {
    ...
    if (schema.kind === "conditional") {
        if (value === undefined) {
            return (0,index_cd5679e3_node_react_server_esm.l)(schema);
        }
        try {
            if (typeof value !== "object" || value === null || isArray(value)) {
                throw new index_cd5679e3_node_react_server_esm.F("Must be an object");
            }
            for (const key of Object.keys(value)){
                if (key !== "discriminant" && key !== "value") {
                    throw new index_cd5679e3_node_react_server_esm.F(`Must only contain keys "discriminant" and "value", not "${key}"`);
                }
            }
        } catch (err) {
            throw new PropValidationError(err, path, schema);
        ...
        try {
            ...
            for (const key of Object.keys(value)){
                if (!allowedKeysSet.has(key)) {
                    throw new index_cd5679e3_node_react_server_esm.F(`Key on object value "${key}" is not allowed`);
                }
            }
        } catch (err) {
            throw new PropValidationError(err, path, schema);
    ...
    if (schema.kind === "array") {
        ...
        try {
            if (!isArray(value)) {
                throw new index_cd5679e3_node_react_server_esm.F("Must be an array");
            }
        } catch (err) {
            throw new PropValidationError(err, path, schema);
        }
...
function formatFormDataError(error) {
    ...
    return flatErrors.map((error)=>{
        if (error instanceof PropValidationError) {
            ...
            return `${path}: ${error.cause instanceof index_cd5679e3_node_react_server_esm.F ? error.cause.message : `Unexpected error: ${error.cause}`}`;
        }
...
var emery_assertions_cjs = __webpack_require__(99577);
...
const listCollection = cache(async function listCollection(repoPath, collectionPath, glob, formatInfo, extension) {
    ...
    return (await Promise.all(entries.map(async (x)=>{
        if (formatInfo.dataLocation === "index") {
            ...
            try {
                await promises_default().stat(external_path_default().join(repoPath, (0,index_cd5679e3_node_react_server_esm.q)(`${collectionPath}/${x.name}`, formatInfo)));
                return [
                    x.name
                ];
            } catch (err) {
...
function collectionReader(repoPath, collection, config) {
    const formatInfo = (0,index_cd5679e3_node_react_server_esm.y)(config, collection);
    const collectionPath = (0,index_cd5679e3_node_react_server_esm.f)(config, collection);
    const collectionConfig = config.collections[collection];
    const schema = (0,index_cd5679e3_node_react_server_esm.o)(collectionConfig.schema);
    const glob = (0,index_cd5679e3_node_react_server_esm.z)(config, collection);
    const extension = (0,index_cd5679e3_node_react_server_esm.B)(formatInfo);
    const read = (slug, ...args)=>{
        var _args$;
        return readItem(schema, formatInfo, (0,index_cd5679e3_node_react_server_esm.A)(config, collection, slug), repoPath, (_args$ = args[0]) === null || _args$ === void 0 ? void 0 : _args$.resolveLinkedFiles, `"${slug}" in collection "${collection}"`, slug, collectionConfig.slugField, glob);
    };
...
const readItem = cache(async function readItem(rootSchema, formatInfo, itemDir, repoPath, resolveLinkedFiles, debugReference, ...slugInfo) {
    let dataFile;
    try {
        dataFile = await promises_default().readFile(external_path_default().resolve(repoPath, (0,index_cd5679e3_node_react_server_esm.q)(itemDir, formatInfo)));
    } catch (err) {
        if (err.code === "ENOENT") {
            return null;
        }
    ...
    try {
        ...
        if (contentFieldPathsToEagerlyResolve !== null && contentFieldPathsToEagerlyResolve !== void 0 && contentFieldPathsToEagerlyResolve.length) {
            await Promise.all(contentFieldPathsToEagerlyResolve.map(async (path)=>{
                const parentValue = (0,index_cd5679e3_node_react_server_esm.P)(validated, path.slice(0, -1));
...
function singletonReader(repoPath, singleton, config) {
    const formatInfo = (0,index_cd5679e3_node_react_server_esm.E)(config, singleton);
    const singletonPath = (0,index_cd5679e3_node_react_server_esm.a)(config, singleton);
    const schema = (0,index_cd5679e3_node_react_server_esm.o)(config.singletons[singleton].schema);
    const read = (...args)=>{
        var _args$2;
